...
public class MovieViewModel extends ViewModel {
    ...
    private final MutableLiveData<Integer> currentPage = new MutableLiveData<>(1); // To track the current page
    ...
    public void searchMovie(String movieTitle, int page) {
        ...
        try {
            // Construct the URL with additional data for ratings and plot
            ...
            ApiClient.get(urlString, new Callback() {
                ...
                public void onResponse(Call call, Response response) throws IOException {
                    ...
                    try {
                        ...
                        // Check the total results and calculate total pages
                        int totalResults = Integer.parseInt(json.getString("totalResults"));
                        int totalPagesValue = (int) Math.ceil(totalResults / 10.0); // 10 results per page
                        totalPages.postValue(totalPagesValue); // Update total pages

                        JSONArray searchResults = json.getJSONArray("Search");
                        List<MovieModel.Movie> moviesList = new ArrayList<>();

                        // Process only 10 results per page
                        for (int i = 0; i < searchResults.length(); i++) {
                            if (i >= 10) break;  // Ensure we're only processing 10 results per page
                            JSONObject movieJson = searchResults.getJSONObject(i);
                            String title = movieJson.getString("Title");
                            String year = movieJson.getString("Year");
                            String imdbID = movieJson.getString("imdbID"); // Required for fetching details

                            // Fetch additional details (plot, ratings, studio, posterURL) from another API request
                            ...
                            ApiClient.get(detailsUrl, new Callback() {
                                ...
                                public void onResponse(Call call, Response response) throws IOException {
                                    ...
                                    try {
                                        ...
                                        // Get the poster URL
                                        String posterURL = detailsJson.optString("Poster", ""); // Empty string if no poster URL available

                                        // Create movie object with all required fields including posterURL
                                        MovieModel.Movie movie = new MovieModel.Movie(title, year, studio, plot, ratingsList, posterURL);
                                        moviesList.add(movie);
                                        movieData.postValue(moviesList); // Update LiveData
